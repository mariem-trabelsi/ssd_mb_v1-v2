...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            if 'optimizer_state_dict' in checkpoint:
                ...
                # Fix for initial_lr KeyError
                for param_group in optimizer.param_groups:
                    if 'initial_lr' not in param_group:
                        param_group['initial_lr'] = param_group['lr']
            
            last_epoch = checkpoint.get('epoch', -1)
            logging.info(f"Resuming training from epoch {last_epoch + 1} with full training state")
        else:
            # Old format: just weights
            net.load_state_dict(checkpoint)
            optimizer = torch.optim.SGD(params, lr=args.lr, momentum=args.momentum,
                                      weight_decay=args.weight_decay)
            epoch_match = re.search(r'Epoch-(\d+)', args.resume)
            last_epoch = int(epoch_match.group(1)) if epoch_match else -1
            logging.info(f"Resuming model weights from epoch {last_epoch + 1} (optimizer state reset)")
        ...
        # No checkpoint provided, create new optimizer
        optimizer = torch.optim.SGD(params, lr=args.lr, momentum=args.momentum,
                                  weight_decay=args.weight_decay)
        last_epoch = -1
    ...
    # define loss function and optimizer (if not already defined)
    if 'optimizer' not in locals():
        optimizer = torch.optim.SGD(params, lr=args.lr, momentum=args.momentum,
                                  weight_decay=args.weight_decay)

    # Ensure all param groups have initial_lr for scheduler compatibility
    for param_group in optimizer.param_groups:
        if 'initial_lr' not in param_group:
            param_group['initial_lr'] = param_group['lr']
    ...
    if args.resume and isinstance(checkpoint, dict) and 'scheduler_state_dict' in checkpoint:
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            logging.info(f"Epoch {epoch}: Validation Loss {val_loss:.4f}, "
                         f"Regression Loss {val_reg_loss:.4f}, Classification Loss {val_clf_loss:.4f}")
            
            tensorboard.add_scalar('Loss/val', val_loss, epoch)
            tensorboard.add_scalar('Regression Loss/val', val_reg_loss, epoch)
            tensorboard.add_scalar('Classification Loss/val', val_clf_loss, epoch)

            if args.validation_mean_ap:
                mean_ap = eval.compute_mean_ap()
                logging.info(f"Epoch {epoch}: Mean Average Precision {mean_ap:.4f}")
                tensorboard.add_scalar('MeanAP/val', mean_ap, epoch)
